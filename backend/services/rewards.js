...
function checkAchievements(db, userId) {
  ...
  const existing = db.prepare('SELECT badge_type FROM achievements WHERE user_id = ?').all(userId).map(a => a.badge_type);
  const newBadges = [];
  ...
  const BADGE_RULES = [
    ...
    },
    {
      ...
      check: () => {
        const count = db.prepare("SELECT COUNT(*) as c FROM missions m JOIN issues i ON m.issue_id = i.id WHERE m.assignee_id = ? AND i.type = 'waste' AND m.status = 'completed'").get(userId);
        return count && count.c >= 3;
      }
    },
    {
      type: 'road_guardian',
      ...
      check: () => {
        const count = db.prepare("SELECT COUNT(*) as c FROM missions m JOIN issues i ON m.issue_id = i.id WHERE m.assignee_id = ? AND i.type = 'cracked_road' AND m.status = 'completed'").get(userId);
        return count && count.c >= 3;
      }
    },
    {
      ...
      check: () => {
        const count = db.prepare("SELECT COUNT(*) as c FROM missions m JOIN issues i ON m.issue_id = i.id WHERE m.assignee_id = ? AND i.type = 'water_leak' AND m.status = 'completed'").get(userId);
        return count && count.c >= 3;
      }
    },
    {
      ...
      check: () => {
        const count = db.prepare("SELECT COUNT(*) as c FROM missions m JOIN issues i ON m.issue_id = i.id WHERE m.assignee_id = ? AND i.type = 'broken_light' AND m.status = 'completed'").get(userId);
        return count && count.c >= 3;
      }
    },
    {
      ...
      check: () => {
        const count = db.prepare("SELECT COUNT(*) as c FROM missions m JOIN issues i ON m.issue_id = i.id WHERE m.assignee_id = ? AND i.type = 'infrastructure' AND m.status = 'completed'").get(userId);
        return count && count.c >= 3;
      }
    },
    {
  ...
  const insertAchievement = db.prepare(`
    INSERT OR IGNORE INTO achievements (id, user_id, badge_type, badge_name, badge_description)
    VALUES (?, ?, ?, ?, ?)
  `);

  for (const rule of BADGE_RULES) {
    if (!existing.includes(rule.type) && rule.check()) {
      const id = `ach-${userId}-${rule.type}-${Date.now()}`;
      insertAchievement.run(id, userId, rule.type, rule.name, rule.description);
 ...
 */
function awardXP(db, userId, amount, reason, refId = '') {
  ...
  db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?)
  `).run(txId, userId, amount, reason, refId || '');
  ...
  db.prepare(`
    UPDATE users SET level = ?, rank = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?
  `).run(newLevel, newRank, userId);

  // Check for new achievements
  const newBadges = checkAchievements(db, userId);
